...
Updated September 9, 2026. The current build is a working local product prototype, with account-scoped persistence and a controlled CALL-E adapter. A CLI test and two app-originated inquiries completed; the second was recovered after a lost create response. Broader product behavior and public production readiness still need validation.
...
The uncertain test has been recovered by one exact-body, same-key operator replay under the documented CALL-E recovery contract. The task returned completed with one attempt and 35 transcript turns; the app now holds its ID and three source-supported facts for review. The transcript confirms the $1,000 quote and continued questioning against a $40 cap; the extracted monetary value was correct. No replacement call was requested. Calling is disabled.
